...
from src.data.dataset.AudioDatasetV2 import AudioDatasetV2


class AudioDataModuleV3(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None) -> None:
        if stage == "fit":
            self.train_dataset = AudioDatasetV2(paths=self.train_paths)
            self.val_dataset = AudioDatasetV2(paths=self.val_paths)
        if stage == "test":
            self.test_dataset = AudioDatasetV2(paths=self.test_paths)
