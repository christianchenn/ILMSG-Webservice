...
class AudioAEV21(pl.LightningModule):
    def __init__(self, learning_rate, run_name):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                in_channels=50, out_channels=25, kernel_size=2
            ),
            nn.BatchNorm2d(25),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=25, out_channels=25, kernel_size=2
            ),
            nn.BatchNorm2d(25),
            # nn.ReLU(),
            # nn.Sigmoid()
    ...
    def normalize(self, tensor, new_min=0.0, new_max=1.0, current_max=None, current_min=None):
        if current_max == None or current_min == None:
            current_max, current_min = tensor.max(), tensor.min()
        
        tensor_p = (tensor - current_min)/(current_max - current_min)
        return tensor_p
    def forward(self, x):
        x = self.encoder(x)
        x = self.normalize(x)
