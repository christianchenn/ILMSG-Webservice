...
class AudioAEV26(pl.LightningModule):
    def __init__(self, learning_rate, run_name):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),            
            nn.Conv2d(
                in_channels=8, out_channels=2, kernel_size=3
            ),
            nn.BatchNorm2d(2),
            nn.ReLU(),
            # nn.Flatten(),
            # nn.Linear(
            #     in_features=2240, out_features=2048
            # ),
            # (8, 118, 16)
        )
        self.decoder = nn.Sequential(
            # nn.Linear(
            #     in_features=2048, out_features=2240
            # ),
            # nn.Unflatten(-1, (2, 112, 10)),
            nn.ConvTranspose2d(
                in_channels=2,
                out_channels=8,
                kernel_size=3,
