...
from src.models.pretrained.VGGFace import VGGFace



# Input 128x128 Transform 4
# Menggunakan Attention dan VGGFace
# Mengunakan power to db
# Target Audio (,2048)
class Vid2SpeechV35(pl.LightningModule):
    def __init__(self, run_name, learning_rate=1e-4, yaml_file=None):
        ...
        self.future_len = 20 
        pretrained = VGGFace().float()
        pretrained_model_dict = torch.load('models/pretrained/vggface.pth', map_location=lambda storage, loc: storage)
        pretrained.load_state_dict(pretrained_model_dict)
        pretrained_model = extract_layers(pretrained.features, 0, 31)
        self.pretrained_model = freeze_layers(pretrained_model, "26.weight")
        ...
        #     nn.ReLU(),
        # )
        self.attention = MultiheadAttention2D(in_channels=512, embed_dim=512, num_heads=8)
        # Encoders
        self.conv_lstm_encoders = []
        self.conv_lstm_encoders.append(
            ConvLSTMCell(input_dim=512,
                         hidden_dim=128,
                         kernel_size=(3, 3),
                         bias=True)
        )
        self.conv_lstm_encoders.append(
            ConvLSTMCell(input_dim=128,
                         hidden_dim=128,
                         kernel_size=(3, 3),
                         bias=True)
        )
