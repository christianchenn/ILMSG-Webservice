...
# Target Audio (,2048)
class Vid2SpeechV42(pl.LightningModule):
    def __init__(self, run_name, learning_rate=1e-4, yaml_file=None):
        ...
        self.attention_2 = MultiheadAttention2D(in_channels=256, embed_dim=256, num_heads=4, mask=True)       
        self.attention_3 = MultiheadAttention2D(in_channels=256, embed_dim=256, num_heads=4, mask=True)       
        self.attention_4 = MultiheadAttention2D(in_channels=256, embed_dim=256, num_heads=4, mask=True)       
    ...
    def forward(self, x):
        ...
        attention_features = []
        for time_frame in range(num_frames):
            feature = self.attention_3(x[:, :, time_frame, :, :])
            attention_features.append(feature)
        x = torch.stack(attention_features, 2)
        
        attention_features = []
        for time_frame in range(num_frames):
            feature = self.attention_4(x[:, :, time_frame, :, :])
            attention_features.append(feature)
        x = torch.stack(attention_features, 2)
