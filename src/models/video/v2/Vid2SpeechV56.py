...
# Target Audio (,2048)
class Vid2SpeechV56(pl.LightningModule):
    def __init__(self, run_name, learning_rate=1e-4, yaml_file=None):
        ...
        self.conv_lstm_1 = ConvLSTM(in_channel=160,
                                    hidden_dim=64,
                                    out_channel=128,
    ...
    def on_load_checkpoint(self, checkpoint) -> None:
            ...
            #     else:
            #         print(key)
            #         # Modify the keys to match the keys in the old_state_dict
            #         old_key = key.replace('encoders.', 'encoders[').replace('decoders.', 'decoders[').replace('.', '].', 1) + ']'
            #         print(old_state_dict[old_key])
            #         if old_key in old_state_dict:
            #             new_state_dict[key] = old_state_dict[old_key]
