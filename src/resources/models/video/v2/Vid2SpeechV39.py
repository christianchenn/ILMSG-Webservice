...
# Target Audio (,2048)
class Vid2SpeechV39(pl.LightningModule):
    def __init__(self, run_name, learning_rate=1e-4, yaml_file=None):
        ...
        self.future_len = 20 
        pretrained_model = extract_layers(efficientnet_v2_s(weights=EfficientNet_V2_S_Weights.IMAGENET1K_V1).features, 0, 7)
        self.pretrained_model = freeze_layers(pretrained_model, "6.5.block.0.0.weight")
        ...
        self.attention_1 = MultiheadAttention2D(in_channels=256, embed_dim=256, num_heads=4)
        ...
        self.attention_2 = MultiheadAttention2D(in_channels=128, embed_dim=256, num_heads=4)
    ...
    def forward(self, x):
        ...
        # print("SIZE BEFORE", x.size())
        for time_frame in range(num_frames):
            feature = self.attention_2(x[:, :, time_frame, :, :])
            attention_features.append(feature)
        x = torch.stack(attention_features, 2)
